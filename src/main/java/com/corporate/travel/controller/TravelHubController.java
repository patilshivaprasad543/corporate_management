...
public class TravelHubController {
    ...
    public ResponseEntity<ApiResponse<TravelHubDto.TravelHubResponse>> getHub(
        ...
        TravelRequest request = loadAuthorizedRequest(travelRequestId, userId);
        return ResponseEntity.ok(ApiResponse.ok(toHub(request), "Travel hub retrieved"));
    }

    @PostMapping("/{travelRequestId}/budget")
    public ResponseEntity<ApiResponse<TravelHubDto.BudgetResponse>> allocateBudget(
            @AuthenticationPrincipal UserPrincipal principal, @PathVariable Long travelRequestId,
            @Valid @RequestBody TravelHubDto.BudgetRequest body) {
        Long userId = AuthenticatedUser.requireId(principal);
        requireRole(principal, "ROLE_HR", "ROLE_COMPANY_ADMIN", "ROLE_SUPER_ADMIN");
        ...
        budget.setFlightLimit(zero(body.flightLimit())); budget.setHotelLimit(zero(body.hotelLimit()));
        budget.setCabLimit(zero(body.cabLimit())); budget.setMealLimit(zero(body.mealLimit())); budget.setOtherLimit(zero(body.otherLimit()));
        budget.setAllocatedBy(userRepository.findById(userId).orElseThrow());
        budget.setStatus(TravelBudget.BudgetStatus.ALLOCATED);
    ...
    public ResponseEntity<ApiResponse<TravelHubDto.AgentContactResponse>> saveAgentContact(
            @AuthenticationPrincipal UserPrincipal principal, @PathVariable Long travelRequestId,
            @Valid @RequestBody TravelHubDto.AgentContactRequest body) {
        Long userId = AuthenticatedUser.requireId(principal); requireRole(principal, "ROLE_COMPANY_ADMIN", "ROLE_SUPER_ADMIN", "ROLE_SUPPORT");
        TravelRequest request = requestRepository.findById(travelRequestId).orElseThrow();
        TravelAgentContact c = contactRepository.findByTravelRequestId(travelRequestId).orElseGet(TravelAgentContact::new);
        c.setTravelRequest(request); c.setAgencyName(body.agencyName()); c.setAgentName(body.agentName()); c.setPhoneNumber(body.phoneNumber());
        c.setEmail(body.email()); c.setEmergencyPhone(body.emergencyPhone()); c.setNotes(body.notes());
    ...
    public ResponseEntity<ApiResponse<TravelHubDto.ChatMessageResponse>> sendChat(
            @AuthenticationPrincipal UserPrincipal principal, @PathVariable Long travelRequestId,
            @Valid @RequestBody TravelHubDto.ChatRequest body) {
        Long userId = AuthenticatedUser.requireId(principal); TravelRequest request = loadAuthorizedRequest(travelRequestId, userId);
        TravelChatMessage m = new TravelChatMessage(); m.setTravelRequest(request); m.setSender(userRepository.findById(userId).orElseThrow());
        m.setMessage(body.message()); m.setAttachmentName(body.attachmentName()); m.setAttachmentKey(body.attachmentKey());
    ...
    private TravelRequest loadAuthorizedRequest(Long id, Long userId) {
        ...
        boolean privileged = userRepository.findById(userId).map(u -> u.getRoles().stream().anyMatch(role ->
            List.of("ROLE_COMPANY_ADMIN","ROLE_SUPER_ADMIN","ROLE_HR","ROLE_FINANCE","ROLE_SUPPORT","ROLE_APPROVER","ROLE_TRAVEL_MANAGER").contains(role.getName().name()))).orElse(false);
        if (!owner && !privileged) throw new AccessDeniedException("You are not authorized for this travel hub");
        return r;
    }
    private void requireRole(UserPrincipal p, String... roles) {
        boolean ok = p.getAuthorities().stream().map(GrantedAuthority::getAuthority).anyMatch(a -> List.of(roles).contains(a));
        if (!ok) throw new AccessDeniedException("Required role is not authorized");
    }
